...
window::Font::Font(int size) : wxFont(size, wxROMAN, wxNORMAL, wxFONTWEIGHT_BOLD) {}
...
window::ListBox* window::ListBox::set_font(const Font& font) {
    wxASSERT_MSG(win, _("call set_font only after create()"));
    static_cast<wxShylockListbox*>(win)->SetFont(font);
    return this;
}

int window::ListBox::which() {
    wxASSERT_MSG(win, _("call which only after create()"));
...
window::Text* window::Text::create(wxWindow* parent_value, const window::Size& size_value, 
    ...
    win = new wxShylockText(parent_value, wxID_ANY, text, 
                            wxDefaultPosition, size_value);
    wxColour background(245, 245, 245);
    Window::create(parent_value, size_value, sizer_value, proportion, flag, border);
    return this;
}
